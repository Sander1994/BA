\newcommand{\startHSCdocument}[1][]{

  % Register different entries: glossary, nomenclature, acronyms
  \makeglossaries
  \include{Verzeichnisse/Worttrennungen}
  \include{Verzeichnisse/Abkuerzungsverzeichnis}
  \makenomenclature
  \include{Verzeichnisse/Symbolverzeichnis}
  \include{Verzeichnisse/Glossar}
  \glsaddall

  % Register actual document
  \begin{document}
  \shorthandoff{"}

  % ===========================================================================
  %                             Document header
  % ===========================================================================

  \newgeometry{
    left=2.5cm,
    right=2.5cm,
    top=2.5cm,
    bottom=2.5cm
  }

  % Correctly determine type of document and format in respective header
  \newcommand{\DocumentType}{#1}

  \begin{titlepage}
    \ifthenelse{\equal{#1}{Praxisbericht}}
      {\include{framework/Header_Practice_Report}}
      {
        \ifthenelse{\equal{#1}{Bachelorarbeit}}
          {\include{framework/Header}}
          {
            \ifthenelse{\equal{#1}{Masterarbeit}}
              {\include{framework/Header}}
              {
                \IfFileExists{./CustomHeader.tex}{
                  \include{CustomHeader}
                }{
                  ERROR: Please use one of the following values as parameter
                  to the HSCdocument environment:\\
                  "Praxisbericht", "Bachelorarbeit", "Masterarbeit"\\

                  To create your own document header, just create a file
                  "CustomHeader.tex" in the document root.
                }
              }
          }
      }
  \end{titlepage}

  \restoregeometry

  \begin{abstract}
  	\raggedright
  	Ziel dieser Arbeit war es, eine Künstliche Intelligenz, für das Brettspiel \textit{"Ganz schön clever"}, zu entwickeln und den Entwicklungs- sowie Trainingsprozess zu evaluieren.
  	Diese Künstliche Intelligenz wurde erfolgreich entwickelt und kann das Brettspiel \textit{"Ganz schön clever"} gut spielen. Zunächst wurde dafür ein Prototyp entwickelt, welcher dann schrittweise erweitert wurde, um die gesamte Komplexität des Spiels zu erfassen.
  	
  	Das Brettspiel \textit{"Ganz schön clever"} ist ein Würfelspiel, welches eine hohe Komplexität aufweist. Diese kommt vor allem durch die vielen Aktionsmöglichkeiten des Spielers und die multiplen Zusammenhängen innerhalb des Belohnungssystems zustande. Außerdem weist es eine hohe Stochastizität auf, was seine Komplexität zusätzlich erhöht.
  	
  	Für die Implementierung der Künstlichen Intelligenz wurde Deep Reinforcement Learning verwendet. Während der Implementierungsphase kam es zunächst zu einem Problem, bei dem die Künstliche Intelligenz es nur schwer selbstständig schaffte, gültige Aktionen zu wählen, welche nicht zum Abbruch des Spiels geführt haben. Dieses Vorgehen, bei dem das Spiel nach Wahl einer ungültigen Aktion abgebrochen wurde, wurde nach einigen Tagen verworfen. Daraufhin wurde zunächst ein Verfahren implementiert, bei dem das Spiel bei ungültigen Aktionen nicht sofort abgebrochen, sondern negativ belohnt wurde. Diese Vorgehen stellte sich als gut geeignet heraus, wurde aber schnell von einem Vorgehen, welches eine Aktionsmaske verwendet abgelöst. Im Folgenden wurde eine Aktionsmaske implementiert, welche gewährleistet, dass nur gültige Aktionen gewählt werden können. Die Performanz stieg mit diesem Vorgehen von vorher durchschnittlich ungefähr 60 Prozent der Maximalpunktzahl auf 75 Prozent. Zudem ergaben sich Schwierigkeiten bei der Erweiterung des Runden-Systems und viele weitere Probleme, die behoben werden mussten.
  	
  	Das Ergebnis des finalen Trainings war eine Künstliche Intelligenz, welche durchschnittlich 204,98 Punkte mit einer Standardabweichung von 31,22 im Spiel erzielt. Im Vergleich dazu erzielten menschliche Spieler in einem Test im Durchschnitt lediglich 175,75 Punkte.
  \end{abstract}
  \renewcommand{\abstractname}{Abstract}
  \newpage
  \begin{abstract}
  	\raggedright
  	The goal of this thesis was to develop an Artificial Intelligence for the board game \textit{"Ganz schön clever"}, and to evaluate the development and training process. This Artificial Intelligence was successfully developed and can play the board game \textit{"Ganz schön clever"} well. Initially, a prototype was developed, which was then gradually expanded to capture the entire complexity of the game.
  	
  	The board game \textit{"Ganz schön clever"} is a dice game that has a high level of complexity. This is mainly due to the many action possibilities of the player and the multiple interconnections within the reward system. In addition, it exhibits a high level of stochasticity, which further increases its complexity.
  	
  	Deep Reinforcement Learning was used for the implementation of the Artificial Intelligence. During the implementation phase, an initial problem occurred where the Artificial Intelligence struggled to autonomously select valid actions that did not lead to the termination of the game. This approach, in which the game was terminated after selecting an invalid action, was discarded after a few days. Subsequently, a method was implemented where the game was not immediately terminated upon invalid actions but instead negatively rewarded. This approach proved to be well-suited but was quickly replaced by a method using an action mask. An action mask was then implemented, ensuring that only valid actions could be chosen. The performance increased with this approach from previously averaging about 60 percent of the maximum score to 75 percent. In addition, there were difficulties in expanding the round system and many other problems that had to be resolved.
  	
  	The result of the final training was an Artificial Intelligence that achieves an average of 204,98 points with a standard deviation of 31,22 in the game. In comparison, human players scored an average of only 175,75 points in a test.
  \end{abstract}
  % ===========================================================================
  %                              Page Ordering
  % ===========================================================================

  % Set image path to "Bilder" subdirectory
  \graphicspath{{Bilder/}}

  \newpage
  \tableofcontents

  \iftotalfigures
    \newpage
    \phantomsection
    \addcontentsline{toc}{section}{\listfigurename}
    \listoffigures
  \fi

  \iftotaltables
    \newpage
    \phantomsection
    \addcontentsline{toc}{section}{\listtablename}
    \listoftables
  \fi

  \iftotallstlistings
    \newpage
    \renewcommand{\lstlistlistingname}{Codebeispielverzeichnis}
    \phantomsection
    \addcontentsline{toc}{section}{\lstlistlistingname}
    \lstlistoflistings
  \fi

  % Nomenclature ("Symbolverzeichnis")
  \iftotalcountnomens
    \newpage
    \phantomsection
    \addcontentsline{toc}{section}{\nomname}
    \printnomenclature[1in]
  \fi

  % Acronyms
  %\iftotalcountacronyms
    \newpage
    \setglossarystyle{super}
    \printglossary[type=\acronymtype,title=Abkürzungsverzeichnis]
  %\fi

  \newpage
}

\newcommand{\finishHSCdocument}{
  % Bibliography
  \newpage
  \bibliographystyle{alphadin}
  \renewcommand{\refname}{Literaturverzeichnis}
  \phantomsection
  \addcontentsline{toc}{section}{\refname}
  \bibliography{Verzeichnisse/Literaturverzeichnis}

  % Glossary
  %\iftotalcountglossarys
    \newpage
    \setglossarystyle{altlist}
    \printglossary
  %\fi

  % Appendix
  \IfFileExists{./Sektionen/Anhang.tex}{
    \newpage
    \pagestyle{empty}
    \appendix
    \renewcommand{\thesection}{A\arabic{section}}
    \include{Sektionen/Anhang}
  }{}

  % Declaration of Honor
  \newpage
  \phantomsection
  \addcontentsline{toc}{section}{Ehrenwörtliche Erklärung}
  \lhead{Ehrenwörtliche Erklärung}
  \include{framework/Declaration_of_Honor}

  % TODO list
  \iftotalcounttodos
    \listoftodos[TODOs]
  \fi

  \end{document}
}

% ===========================================================================
%                   Simplified centered & colored tables
% ===========================================================================

\newenvironment{colortable}[1]{
  \begin{center}
    \begin{tabular}{#1}
    \hline
    \rowcolor{Gray}
}
{
    \hline
    \end{tabular}
  \end{center}
}

\newcommand{\tablecontent}{
  \hline
  \rowcolor{White}
}
